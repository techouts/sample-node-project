...
import { cartState, SSBLogos, userState } from "../../recoilstore";
...
export function StandardDeliveryOptions({
  ...
  const [standardDeliveryAddressId, setStandardDeliveryAddressId] = useState(0);
  ...
  const updateStandardDeliveryAddressId = () => {
    setStandardDeliveryAddressId(
  ...
  useEffect(() => {
    updateStandardDeliveryAddressId();
  }, [cartStore?.userAddresses?.addresses, cartStore?.currentDeliveryMode]);
  ...
  useEffect(() => {
    const addressData = cartStore?.userAddresses?.addresses?.filter(
      (address: any) => address.id === standardDeliveryAddressId
    );
  ...
  }, [standardDeliveryAddressId, cartStore?.userAddresses]);
  ...
  const handleAddressChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const changePincode = async () => {
      ...
      if (response?.error) {
        ...
        setStandardDeliveryAddressId(id);
      }
    };
    changePincode();
  };
      ...
      <SecondaryBox>
        ...
        <FormControlStyled isMobile={isMobile}>
          <StyledRadioGroup
            ...
            value={standardDeliveryAddressId}
          ...
          >
            ...
            {otherAddress && (
              <>
                ...
                {otherAddress?.map((addressItem: any, index: any) => (
