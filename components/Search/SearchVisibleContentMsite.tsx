...
import { No_RESULTS_SEARCH_ICON, RECENT_SEARCH_ICON } from "../../utility/AppIcons";
...
const SearchContentMsite = ({
  ...
  const autoLoadSuggestions = (query: any) => {
    ...
    if (!IS_UNBXD_ENABLED) {
      client
        ...
        .then((response) => {
          const hasError = handleErrorResponse(
            response?.data?.ProductSuggestions?.products
          ); //response checking
      ...
      axios(
        IS_UNBXD_MIDDLEWARE_ENABLED
          ? `${MIDDLEWARE_UNBXD}/autosuggest?q=${encodeURIComponent(query)}`
          : `/api/getSearchSuggestionsV2?q=${encodeURIComponent(query)}`
      )
        .then((response) => {
              ...
              <Grid lg={12}>
                {!spinnerLoaderSearch && search.length !== 0 && (
                  <Grid>
                    <Box
                      ...
                      {filterContent?.length <= 0 && isMobile && (
                        <Box width={"100%"}>
                ...
                {isMobile && !userDataItems?.storeMode && (
                  <Stack gap={isMobile ? "3rem" : "2rem"}>
                    {/* popular categories */}
                    {(popularCategories?.items?.map((item: any) => ({
                      ...item,
                    }))?.length > 0 ||
                      ...
                      <GridImage
                        style={{
                          gridTemplateColumns: isMobile
                            ? `repeat(3 ,1fr)`
                            : `repeat(${data.popularcolumns} ,1fr)`,
                        }}>
...
export default SearchContentMsite;
