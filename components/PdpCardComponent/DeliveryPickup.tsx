...
function DeliveryPickup({
  ...
  function checkServiceability(lat: number, long: number, pincode: any) {
    setLoader(true);
    const address = {
    ...
    graphql
      ...
      .then((response: any) => {
        let data = response?.data?.checkAvailabilityAndEDD?.products;
        const _status = response?.data?.checkAvailabilityAndEDD?.status || "";
        handleServiceabilityResponse({data,_status});
      })
      .catch((error: any) => {
