...
export default function UnbxdNestedCategoriesFilters(props: any) {
  const { data, categoryPath, setCategoryPath } = props;
  const [isExpanded, setExpand] = useState(categoryPath !== undefined);
  ...
  function handleOnClick(path: string) {
    setCategoryPath(path === "" ? null : encodeURIComponent(path));
  ...
  return (
    <MainAccordion aria-label="MainAccordion" expanded={isExpanded}>
      <StyledAccordian
        onClick={() => setExpand(!isExpanded)}
        expandIcon={
          <img src={`${ReplaceImage(ARROWDOWN?.url)}`} alt="ArrowDownIcon" />
        }
      ...
      >
        <PageTitle ischeckedfilter={isExpanded}>{data?.label}</PageTitle>
      </StyledAccordian>
        ...
        >
          ...
          >
            <PageTitle
              ...
              sx={{
                textDecoration:
                  currentCat?.name === selectedCatagories?.[selectedCatagories?.length -1]?.name ? "underline" : "none",
