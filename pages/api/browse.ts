...
export default async function handler(
  ...
  const { vcItems = [], filters = {} } = req.body;
  ...
  const generateParams = filterKeys
    ...
    .map((a) => `filter=${encodeURIComponent(a)}`)
    .join('&');
  ...
  const finalModParams = isVC
    ? `q=${searchQuery}&rows=${items}&page=${page}&variants.relevant=true&variants.count=*&filter=${vcFilterParam}${finalParams}${sortQueryParam}`
    : `p=categoryPath:"${searchQuery}"&pagetype=boolean&rows=${items}&page=${page}${categoryParam}&variants.relevant=true${finalParams}${sortQueryParam}`;

  const finalURL = `${UNBXD_HOST}/${secret}/${site_key}/category?${finalModParams}`;
